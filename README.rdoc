...
While Google's Fusion Tables is currently the only implemented one, support for pushing dataset to other remote 
datastores (e.g. {couchDB}(http://couchdb.apache.org/),  {cartoDB}(http://cartodb.com) ), and {CKAN Webstore} (http://wiki.ckan.org/Webstore) is underway.