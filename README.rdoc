...
While Google's Fusion Tables is currently the only one implemented, support for pushing dataset to other remote data stores 
datastores (e.g. {couchDB}[http://couchdb.apache.org/],  {cartoDB}[http://cartodb.com], and {CKAN Webstore}[http://wiki.ckan.org/Webstore] will be added soon.